...
local function update_backdrop(w, c)
    ...
    local shape = cairo.ImageSurface.create(cairo.Format.A1, geo.width,
                                            geo.height)
    ...
    cr:mask_surface(c_shape, c_geo.x + c.border_width - geo.x,
...
_G.client.connect_signal('manage', function(c)
    if c.drawBackdrop == true then backdrop(c) end
end)
